...
export default function Command() {
  ...
  useEffect(() => {
    async function parseRecentlyViewedProjects() {
      try {
        ...
        setRecentlyViewedProjects(recentProjects
          .filter(project => hash[project.url])
          .sort())

        setRecentlyViewedProjectMap(new Map(recentProjects.filter(project => hash[project.url]).map(project => [project.url, project])))
      } catch (error) {
        console.error(error);
      }
    }
 ...
 console.log("recentlyViewedProjects ", recentlyViewedProjects.length);


  return (
    <List navigationTitle="Search Projects" searchBarPlaceholder="Search your Projects" isShowingDetail isLoading={isLoadingRecentProjects}>
      ...
      {recentProjects.length > 0 && !isLoadingRecentProjects ? (
        <List.Section title="Recently Updated Projects">
          {recentProjects.filter(p => !recentlyViewedProjectMap.get(p.url)).map((p) => (
            <List.Item
              key={p.id}
              title={p.title}
              icon={{ source: "table.svg" }}
